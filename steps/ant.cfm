...
<cfif not FileExists("#configObj.get('filelocations','output')#/build.xml") or url.forceANTRefresh>
	...
	<project name="#configObj.get('application','appname')#" default="Rebuild and Refresh">
		<property name="squidheadURL" value="http://#cgi.server_name##cgi.script_name#" />
		...
		<target name="Run Squidhead">
				<echo message="Rebuild Via Squidhead"/>
				<get src="${squidheadURL}?method=run&amp;appName=${appName}" dest="${logPath}\squidheadRefresh.html"/>
		</target>

		<target name="Run Squidhead Full Refresh">
				<echo message="Rebuild Via Squidhead in Full Refresh Mode"/>
				<get src="${squidheadURL}?method=run&amp;appName=${appName}&amp;forceRefresh=TRUE" dest="${logPath}\squidheadFullRefresh.html"/>
		</target>
		...
		<target name="Rebuild and Refresh" depends="Run Squidhead,Refresh Application" />
