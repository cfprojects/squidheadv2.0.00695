...
<cfcomponent output="false">
	...
	<cffunction access="public" name="createComment" output="false" returntype="string" description="Creates a comment string to document the stored procedure.">
		...
		<cfif len(arguments.author) gt 0>
			<cfset results.append(chr(9) & 'Author: #chr(9)# #chr(9)# #chr(9)# #arguments.author# #variables.configObj.get("settings", "squidheadVersion")#' & Chr(13) & Chr(10)) />
		</cfif>
	...
	<cffunction access="private" name="CreateStoredProc" output="false" returntype="boolean" description="Creates a stored proc n a given database.">
		...
		<cfcatch type="any">
			<cfif FindNoCase("The text, ntext, and image data types cannot be compared", cfcatch.detail) >
				...
				<cfthrow message="Problem with Generated Stored Procedure" detail="The stored procedure is trying to sort on a text, ntext, or image. Change the order of the columns table in the table #UCase(tableName)#.<p>#errorText#</p>" type="SQUIDHEAD" />
			</cfif>
			<cfdump var="#cfcatch#">
