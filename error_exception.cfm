...
<cfif StructKeyExists(error, "RootCause") and StructKeyExists(error.RootCause, "type") and FindNoCase("SQUIDHEAD", error.RootCause.type)>
	<cfoutput>
...
<cfelseif isDefined("error.Diagnostics") AND findNoCase("squidhead.cfc.customtag.", error.Diagnostics)>
...
<cfelseif isDefined("error.Diagnostics") AND findNoCase("SELECT command denied", error.Diagnostics) AND findNoCase("for table 'proc'", error.Diagnostics)>
	<p>Squidhead requires that the database account used has <em>SELECT</em> Permission on the table <em>mysql.proc</em>. See documentation for more information.</p>
	<p><cfoutput>#error.rootcause.detail#</cfoutput>
<cfelse>
