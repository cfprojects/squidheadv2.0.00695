...
<dt>What is Squidhead?</dt>
<dd>Squidhead is a ColdFusion application that creates scaffolds for a SQL database.  It creates them using stored procedures instead of inline SQL.
It also automatically creates ColdFusion code for user created stored procedures.</dd>

<dt>Why is it called Squidhead?</dt>
<dd>My wife told me to call it that. It comes from a fake band name I came up with a while back. I figure the SQ in Squidhead works for something with SQL.</dd>
...
Additionally, Squidhead introspects using foreign keys, making reference tables very easy to use.
</dd>
...
<dt>How does Squidhead handle outputs?</dt>
...
<dt>Oh Yeah? How does Squidhead handle multiple queries?</dt>
...
Squidhead will build your CRUD stored procedures respecting deactivation versus deletion. (The list function will obey it too.)
</dd>

<dt>What's the XML Disk Cache? </dt>
<dd>It's an XML Representation of the database structure that Squidhead creates when it analyzes your database. Certain settings in this file override introspected database information.<br /><br />
...
<dt>Does Squidhead support foreign keys?</dt>
<dd>Yes.</dd>
<dt>Care to elaborate on that?</dt>
<dd>
	<p><em>Form UI</em> - On forms that interact with tables with foreign keys, Squidhead will produce a select box of the aforementioned "foreign key labels."</p>
	...
	<p>If you use the application template <em>fkCrazy</em> Squidhead will build methods in all of your business objects that allow you to manipulate tables across foreign key linked tables.</p>
</dd>
